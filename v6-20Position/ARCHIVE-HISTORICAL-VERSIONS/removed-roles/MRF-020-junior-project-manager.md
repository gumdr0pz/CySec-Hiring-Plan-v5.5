# Junior Project Manager

| Field | Detail |
| --- | --- |
| Requisition ID | MRF-020 |
| Business Unit | Cybersecurity BU |
| Team | PM Team |
| Reports To | BU Head (dotted line) |
| Level / Grade | Junior |
| Employment Type | Full-time, Permanent |
| Location | Kuala Lumpur, Malaysia (hybrid) |
| Hiring Wave | Wave 3 — Triggered at RM8M+ recurring revenue |
| Target Start | Within 90 days of trigger |
| Priority | Medium |

## 1. Role Purpose

The Junior Project Manager supports the smooth delivery of cybersecurity engagements across the Cybersecurity BU's offensive security, threat intelligence, DevSecOps, blockchain security, and consulting service lines. Working alongside a delivery lead, this role keeps projects organised, tracked, and well-documented so that delivery teams can focus on technical work and clients receive timely, transparent updates. It is an entry point for building a project management career within a growing, high-demand security practice.

## 2. Key Responsibilities

- Support the delivery lead in day-to-day coordination of cybersecurity projects, ensuring tasks, owners, and timelines stay aligned with the agreed delivery plan.
- Track project milestones, deliverables, and dependencies, flagging slippages or risks early so corrective action can be taken.
- Coordinate team schedules, book and prepare project meetings, and circulate agendas and follow-up actions to keep stakeholders informed.
- Document project status, progress reports, and meeting minutes, maintaining accurate and up-to-date records for internal and client reference.
- Manage project administrative tasks, including maintaining project files, registers, and tracking tools so information is consistent and easy to retrieve.
- Compile regular status updates and reports for the delivery lead and relevant stakeholders.
- Monitor action items and outstanding deliverables, following up with team members to confirm completion.
- Contribute to continuous improvement by keeping project documentation and tooling tidy, current, and aligned with team standards.

## 3. Required Qualifications & Experience

- Bachelor's degree in Business, Computer Science, or a related field.
- At least 1 year of experience in project coordination or a similar support role.
- Basic working knowledge of project management methodologies.
- Strong organisational skills, with the ability to manage multiple tasks and priorities.
- Proficiency in commonly used project management tools.

## 4. Preferred / Desirable Qualifications

- CAPM or an equivalent project management certification.
- Exposure to Agile/Scrum ways of working.
- Familiarity with or interest in cybersecurity projects.
- Solid communication and reporting skills.

## 5. Technical Skills & Competencies

- Hands-on use of project management and collaboration tools (e.g., Jira, Trello, Asana, MS Project, or similar).
- Competence with productivity and documentation suites (e.g., Microsoft 365 / Google Workspace) for reports, schedules, and minutes.
- Understanding of project lifecycle concepts, including planning, tracking, and status reporting.
- Awareness of Agile/Scrum and traditional (waterfall) delivery approaches.
- Basic appreciation of cybersecurity service delivery and associated terminology (an advantage).

## 6. Behavioural Competencies

- Communication: shares clear, timely written and verbal updates with team members and stakeholders.
- Ownership: takes responsibility for assigned tasks and follows through to completion.
- Collaboration: works well within a delivery team and supports colleagues across service lines.
- Attention to detail: maintains accurate, consistent records and documentation.
- Adaptability: responds positively to shifting priorities in a fast-moving project environment.
- Eagerness to learn: actively builds project management and cybersecurity knowledge.

## 7. Certifications

**Required**
- None mandatory.

**Preferred**
- CAPM (Certified Associate in Project Management) or equivalent.

## 8. Key Performance Indicators

- Accurate and up-to-date project trackers, with milestone status maintained for all assigned projects.
- Timely delivery of project status reports and meeting minutes within agreed turnaround times.
- Effective coordination of team schedules and meetings, measured by attendance and on-time scheduling.
- Reduction in overdue or untracked action items across supported projects.
- Positive feedback from the delivery lead and delivery teams on coordination and documentation quality.
- Demonstrated growth in project management knowledge and tooling proficiency within the first 6–12 months.

## 9. Reporting & Team Context

This role reports to the delivery lead and works closely with delivery teams across the Cybersecurity BU's service lines, supporting project coordination, tracking, and documentation. It is an individual-contributor role focused on learning and growing within the PM Team.

## 10. Compensation & Benefits

- Monthly Base Salary: RM3,500 – RM5,600
- Annual Base Salary: RM42,000 – RM67,200

In addition to base salary, we offer statutory EPF, SOCSO, and EIS contributions, medical coverage, training and certification sponsorship, and a performance-based bonus.

## 11. Career Progression

With demonstrated capability and experience, the next step is progression to Project Manager, taking on full ownership of project delivery within the Cybersecurity BU.

## 12. Equal Opportunity Statement

We are an equal opportunity employer. We welcome and consider all qualified applicants regardless of race, religion, gender, age, disability, or any other status protected by applicable law, and we are committed to building an inclusive and diverse workplace.
