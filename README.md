# Cybersecurity BU Hiring Plan - v5.5 (20-Hire Structure)

**Version:** 5.5 ⭐ **CURRENT**
**Date:** June 23, 2026
**Status:** Final - Support-Pod 7-5-8 Structure
**Classification:** CONFIDENTIAL

> **⚠️ Important:** This is the **only current version**. All previous versions (v5.0–v5.4) are **deprecated** and archived for reference only.

---

## Overview

This repository contains the complete **v5.5** hiring plan for the Cybersecurity Business Unit (BU): **20 positions** across **9 teams** over 12 months, in a **support-pod 7-5-8** wave structure. Each wave builds deliverable capability, pairing seniors with junior capacity where it scales.

**📦 What's Current:**
- ✅ v5.5: Support-pod 7-5-8 wave structure (20 hires; 23 total positions)
- ✅ Wave 1: 7 (4 Senior + 3 Junior) | Wave 2: 5 (Senior/Lead) | Wave 3: 8 (1 Senior + 7 Junior/IC)
- ✅ Wave 1 builds PreSales and DevSecOps as senior+junior pods, plus a Technical Account Manager, governance, and GRC

**🗄️ What's Archived / Removed:**
- ❌ v5.0–v5.4: Deprecated, in `v5-20Position/ARCHIVE-HISTORICAL-VERSIONS/`
- ❌ Senior Project Manager & Junior Professional Services roles: removed (`.../removed-roles/`)

---

## Key Metrics

| Metric | Value |
|--------|-------|
| **Total Positions** | 23 |
| **Existing Leadership** | 3 |
| **Positions to Hire** | 20 |
| **Hiring Timeline** | 12 months |
| **Revenue Target** | RM12,000,000 |
| **Wave Structure** | 7-5-8 |
| **Rate Bands** | 10 Senior/Lead + 10 Junior/IC |
| **Total Annual Cost** | RM1,451,520 – RM2,322,480 |
| **Cost as % of Revenue** | 12.1% – 19.4% |
| **25% Revenue Cap** | ✅ Within |
| **Headroom** | RM677,520 – RM1,548,480 |

---

## Wave Structure

| Wave | Trigger | Positions | Level Mix | Annual Cost Range | % of Total |
|------|---------|-----------|-----------|-------------------|------------|
| **Wave 1** | Board Approval | 7 | 4 Senior + 3 Junior | RM520,128 – RM832,224 | 35.8% |
| **Wave 2** | RM4M+ SOWs | 5 | 3 Senior + 2 Lead | RM423,360 – RM677,400 | 29.2% |
| **Wave 3** | RM8M+ Recurring | 8 | 1 Senior + 7 Junior/IC | RM508,032 – RM812,856 | 35.0% |
| **Total** | - | **20** | - | **RM1,451,520 – RM2,322,480** | **100%** |

### 36-Month Budget Runway (Estimated)

Year 1 phased; Years 2–3 full run-rate; revenue flat at RM12M; 25% cap = RM3M/yr.

| Year | Annual Cost (Min–Max) | Cumulative (Max) | Within Cap? |
|------|------------------------|------------------|-------------|
| Year 1 (ramp) | RM652,176 – RM1,043,508 | RM1,043,508 | ✅ |
| Year 2 (full) | RM1,451,520 – RM2,322,480 | RM3,365,988 | ✅ |
| Year 3 (full) | RM1,451,520 – RM2,322,480 | RM5,688,468 | ✅ |

36-month cost ≈ 9.9%–15.8% of cumulative revenue (RM36M); cumulative headroom under the cap RM3.3M–5.4M. Full detail in the Financial Analysis.

### Wave 1 Positions (7)
1. **Presales Consultant** ⭐ (Senior) — Sales  ┐
2. **Jr Presales Consultant** (Junior) — Sales  ┘ *PreSales pod*
3. **Gov Sec PM** (Senior) — Cybersecurity
4. **Sr GRC Specialist** (Senior) — VORON
5. **Sr DevSecOps** (Senior) — DevSecOps  ┐
6. **Jr DevSecOps** (Junior) — DevSecOps  ┘ *DevSecOps pod*
7. **Technical Account Manager** (Junior/Intermediate) — Professional Services

### Wave 2 Positions (5)
Sales Specialist (Enterprise) · Sales Specialist (Government) · Lead Researcher (Lead) · Red Team · CTI Lead (Lead)

### Wave 3 Positions (8)
Sr Blockchain Dev · Jr GRC Specialist · Jr Researcher · TI Analyst · TI Researcher · Dist Intel Analyst · Business Analyst · Jr PM

---

## Job Descriptions (`JDs/`)

| File | Position | Level | Wave |
|------|----------|-------|------|
| `MRF-012-senior-presales-consultant.md` | Presales Consultant ⭐ | Senior | 1 |
| `MRF-022-junior-presales-consultant.md` | Jr Presales Consultant | Junior | 1 |
| `MRF-001-government-security-project-manager.md` | Gov Sec PM | Senior | 1 |
| `MRF-002-senior-grc-specialist.md` | Sr GRC Specialist | Senior | 1 |
| `MRF-003-senior-devsecops-engineer.md` | Sr DevSecOps | Senior | 1 |
| `MRF-018-junior-devsecops-engineer.md` | Jr DevSecOps | Junior | 1 |
| `MRF-005-technical-account-manager.md` | Technical Account Manager | Junior/Intermediate | 1 |
| `MRF-010-senior-sales-specialist-enterprise.md` | Sales Specialist (Enterprise) | Senior | 2 |
| `MRF-011-senior-sales-specialist-government.md` | Sales Specialist (Government) | Senior | 2 |
| `MRF-006-lead-security-researcher.md` | Lead Researcher | Lead | 2 |
| `MRF-007-red-team-specialist.md` | Red Team | Senior | 2 |
| `MRF-008-cyber-threat-intelligence-lead.md` | CTI Lead | Lead | 2 |
| `MRF-009-senior-blockchain-developer.md` | Sr Blockchain Dev | Senior | 3 |
| `MRF-013-junior-grc-specialist.md` | Jr GRC Specialist | Junior | 3 |
| `MRF-014-junior-security-researcher.md` | Jr Researcher | Junior | 3 |
| `MRF-015-threat-intelligence-analyst.md` | TI Analyst | IC | 3 |
| `MRF-016-threat-intelligence-researcher.md` | TI Researcher | IC | 3 |
| `MRF-017-distributed-intelligence-analyst.md` | Dist Intel Analyst | IC | 3 |
| `MRF-019-business-analyst.md` | Business Analyst | IC | 3 |
| `MRF-020-junior-project-manager.md` | Jr PM | Junior | 3 |

**Wave totals:** Wave 1 = 7 · Wave 2 = 5 · Wave 3 = 8 · **Total = 20**
**Retired:** MRF-004 (Senior Project Manager), MRF-021 (Junior Professional Services) — removed from the structure.

---

## Organizational Structure

```
BU Head
├── Principal Cybersecurity Architect
│   ├── Gov Sec PM (Senior) ........................ W1
│   ├── VORON: Sr GRC (W1) → Jr GRC (W3)
│   ├── OffSec: Lead Researcher (W2), Red Team (W2) → Jr Researcher (W3)
│   ├── CTI: CTI Lead (W2) → TI Analyst, TI Researcher, Dist Intel (W3)
│   └── DevSecOps: Sr DevSecOps (W1) → Jr DevSecOps (W1)
├── Principal Blockchain Architect
│   └── Sr Blockchain Dev (Senior) ................ W3
├── PM Team (dotted line) — no senior PM
│   ├── Business Analyst (IC) ..................... W3
│   └── Jr PM (Junior) ........................... W3
├── Sales Team (dotted line)
│   ├── Presales Consultant (Senior) ⭐ → Jr Presales (W1)
│   ├── Sales Specialist – Enterprise (Senior) ... W2
│   └── Sales Specialist – Government (Senior) ... W2
└── Professional Services (dotted line)
    └── Technical Account Manager (Jr/Int) ....... W1
```

---

## COGS Formula

```
Burdened Cost = Base Salary × 1.44x
```

**Components:** COLA 10% · EPF 11% · SOCSO 1.45% · Health RM100/mo · Bonus 10% · PTO 12 hrs

**Salary Bands (30% reduction applied):**
- Senior / Lead: RM4,900–7,840/mo base → RM84,672–135,480/yr burdened
- Junior / IC / Junior-Intermediate: RM3,500–5,600/mo base → RM60,480–96,768/yr burdened

---

## Version History & Deprecation Notice

| Version | Date | Wave Structure | Status |
|---------|------|----------------|--------|
| v5.5 | June 23, 2026 | **7-5-8 (support pods)** | ⭐ **CURRENT - USE THIS** |
| v5.4 | June 23, 2026 | 8-5-8 | ❌ Deprecated |
| v5.3 | June 10, 2026 | 7-7-7 | ❌ Deprecated |
| v5.2 | June 10, 2026 | 8-4-9 (flat) | ❌ Deprecated |
| v5.1 | June 10, 2026 | 5-7-9 | ❌ Deprecated |
| v5.0 | June 10, 2026 | 5-7-9 | ❌ Deprecated |

### ⚠️ Deprecation Notice

**Only v5.5 documents should be used for active hiring.**

- **What's New in v5.5:** Junior Professional Services removed (Wave 1 → 7; Professional Services is a single Technical Account Manager). Builds on v5.4's support-pod model (Senior PM removed, Jr Presales added).
- **Old Versions:** `v5-20Position/ARCHIVE-HISTORICAL-VERSIONS/`

---

## Getting Started

### For Management
1. Start with `CySec-Hiring-Plan-Executive-Summary-v5.5.md`
2. Review `CySec-Financial-Analysis-v5.5.md` for cost details
3. Approve Wave 1 using `CySec-Wave1-Hiring-Plan-v5.5.md`

### For HR
1. Use `CySec-Manpower-Requisitions-v5.5.md` for MRF approvals
2. Post jobs from the `JDs/` folder
3. Reference `CySec-Wave1-Hiring-Plan-v5.5.md` for the Wave 1 timeline

### For Finance
1. Review `CySec-Financial-Analysis-v5.5.md` for detailed costs
2. Run `cysec_cogs_calculator.py` for scenario planning
3. Monitor against the 25% revenue cap

---

## Approval

| Role | Name | Signature | Date |
|------|------|-----------|------|
| BU Head | ________ | __________ | ____/____/____ |
| HR Director | ________ | __________ | ____/____/____ |
| CFO | ________ | __________ | ____/____/____ |
| CEO/COO | ________ | __________ | ____/____/____ |

---

**CONFIDENTIAL** - This document contains sensitive business information. Do not distribute without authorization.
